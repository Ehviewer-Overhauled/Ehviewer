    ...
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < height; y++) {
            ...
            if (y <= tile_height / 2 || y > height - tile_height / 2) {
                p->c1 = clamp((int) (255 * (accumulation[tile_index1][p->c1] * (1 - u) +
                                            accumulation[tile_index2][p->c1] * u)));
                continue;
            }
            ...
            if (x <= tile_width / 2 || x > width - tile_width / 2) {
                p->c1 = clamp((int) (255 * (accumulation[tile_index1][p->c1] * (1 - v) +
                                            accumulation[tile_index3][p->c1] * v)));
                continue;
            }
